...
import { validateVerifyEmail } from '../../../validators/account.validator.js';
...
export default async (req, res) => {
  ...
  try {
    req.account = verify(req.body.token, jwtSecretKey);
  } catch (err) {
    return res.status(400).json(errorHelper('00055', req, err.message));
  }

  const exists = await Account.exists({ _id: req.account._id, isActivated: true })
    .catch((err) => {
  ...
  if (req.body.code !== req.account.code) return res.status(400).json(errorHelper('00054', req));

  await Account.updateOne({ _id: req.account._id }, { $set: { isVerified: true } })
    .catch((err) => {
  ...
  logger('00058', req.account._id, getText('en', '00058'), 'Info', req);
 ...
 *    post:
 *      summary: Verifies the email address of the account.
 *      requestBody:
