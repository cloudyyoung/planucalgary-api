...
import { ProgramCreateRelationsSchema, ProgramListSchema } from "./validators"
...
router.post("/", admin(), zod({ body: ProgramCreateSchema.merge(ProgramCreateRelationsSchema) }), createProgram)
router.put(
  "/:id",
  admin(),
  zod({ params: IdInputSchema, body: ProgramUpdateSchema.merge(ProgramCreateRelationsSchema) }),
  updateProgram,
)
